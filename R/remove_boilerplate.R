...
remove_boilerplate <- function(input_dir, ngram_dir, output_dir, rep_text_dir,
    ...
    for(i_doc in 1:length(docs)) {
        ...
        n_pages <- stringr::str_count(txt, "\\f") + 1
        ...
        if(length(ng_sorted) > 0) {
            for(s in ng_sorted) {
                rplc <- if(stringr::str_detect(s, "\\f")) "\f" else ""
                txt <- stringr::str_replace_all(txt, fixed(s), rplc)
            }
        }
        ...
        token_pattern <- "[\\S]+|\\f"
        page_breaks <- stringr::str_locate_all(txt, "\\f")[[1]][,1]
        all_words <- stringr::str_locate_all(txt, token_pattern)[[1]]
        ...
        header_footer_text <- paste(sapply(page_break_word_nums, function(i) {
            ...
            return(stringr::str_sub(txt, start_loc, stop_loc))
            }
        ), collapse=" ")
        header_footer_text <- stringr::str_replace_all(header_footer_text, "\\f", "")
        # Get ngrams with n 2-40
        flagged <- matrix(nrow=0, ncol=2)
        for(i in 2:40) {
            if(ngram::wordcount(header_footer_text) >= i) {
                ngram_tab <- ngram::get.phrasetable(ngram::ngram(header_footer_text, n=i))
        ...
        if(nrow(flagged) > 0) {
            ...
            #     if(sum(stringr::str_detect(flagged[ , 1], fixed(flagged[i, 1]))) == 1) {
            ...
            flagged_sorted[ , 1] <- stringr::str_replace_all(flagged_sorted[ , 1], "Â", "")
            # Remove it (from around page breaks) if more than 80% of occurrences are within
            # 40 words from page breaks
            counts <- stringr::str_count(txt, fixed(flagged_sorted[,1]))
            ...
            for(i in seq_along(page_break_word_nums)) {
                ...
                if(i==length(page_break_word_nums)) {
                    ...
                    other_text[i+1] <- if(final_page_lt_41_words) "" else stringr::str_sub(txt, all_words[n+41, 1], nchar(txt))
                ...
                if(i==1) {
                    ...
                    if(first_page_lt_41_words) {
                        other_text[1] <- ""
                        header_footer_text[1] <- stringr::str_sub(txt, 1, head_foot_stop)
                        next
                    } else {
                        other_text[1] <- stringr::str_sub(txt, 1, all_words[n-41, 2])
                        header_footer_text[1] <- stringr::str_sub(txt, all_words[n-40, 1], head_foot_stop)
                        next
                    }
                } else {
                    ...
                    other_text[i] <- if(prev_page_lt_81_words) "" else stringr::str_sub(txt, all_words[n_minus_1 + 41, 1], all_words[n-41, 2])
                    header_footer_text[i] <- stringr::str_sub(txt, head_foot_start, head_foot_stop)
                }
            }
            ...
            for(s in to_remove) {
                rplc <- if(stringr::str_detect(s, "\\f")) "\f" else ""
                header_footer_text <- stringr::str_replace_all(header_footer_text, fixed(s), rplc)
            }
